# Simulation Studies for ZIP-LPCM

This tutorial includes the coding for the two simulation studies illusrated in the paper **"A Zero-Inflated Poisson Latent Position Cluster Model" (ZIP-LPCM)**.
...
We implement the supervised and unsupervised ZIP-LPCM and Pois-LPCM inference with default unusual zero probability prior setting, Beta(1,9), for each scenario following **Algorithm 1** of the **ZIP-LPCM** paper. 
...
The corresponding model parameters and latent variables are in line with those introduced in the **ZIP-LPCM** paper.
...
The **Figure 1** in the **ZIP-LPCM** paper can be recovered following the code below where the functions `subplot()` and `orca()` are leveraged to aggregate multiple interactive plots from different specific angles in one figure and to output a high-quality screenshot, respectively.
...
The above code will output exactly the same figure as the **Figure 1** of the **ZIP-LPCM** paper.
...
The adjacency matrices heatmap plots shown in **Figure 2** of the **ZIP-LPCM** paper can be reproduced following the code:
...
The prior parameters are set to be $(\omega=0.01,\alpha_1=1,\alpha_2=0.103,\alpha=3)$ which are all in agreement with those stated in the **ZIP-LPCM** paper.
...
The statistic $`\mathbb{E}(\\{|\hat{d}_{ij}-d^*_{ij}|\\}){\scriptsize [\text{sd}]}`$ in the 5th column of the **Table 1** in the **ZIP-LPCM** paper can be obtained for the **ZIP-LPCM Sup Beta(1,9)** case in **scenario 1** following:
...
The `output$EPL` code above returns the statistic $`\mathbb{E}_{\boldsymbol{z}}[\text{VI}(\hat{\boldsymbol{z}},\boldsymbol{z}) \mid \boldsymbol{Y}]`$ shown in the 4th column of **Table 1** of the **ZIP-LPCM** paper for the **ZIP-LPCM Sup Beta(1,9)** case in **scenario 1**.
...
The code for obtaining the marginal posterior mode of the posterior clustering can also help us obtain $\hat{\boldsymbol{U}}$ following (23) of the **ZIP-LPCM** paper, that is,
...
the value of which is eactly the one we show in the 6th column of **Table 1** of the **ZIP-LPCM** paper for the **ZIP-LPCM Sup Beta(1,9)** case in **scenario 1**.
...
Comparing the $\hat{\boldsymbol{p}}$ with the reference $`\boldsymbol{p}^*`$ gives the statistic $`\mathbb{E}(\{|\hat{p}_{z_iz_j}-p^*_{z_iz_j}|\}){\scriptsize [\text{sd}]}`$ shown in the 7th column of **Table 1** of the **ZIP-LPCM** paper for the **ZIP-LPCM Sup Beta(1,9)** case in **scenario 1**:
...
Finally, we ontain the $\hat{\boldsymbol{\nu}}$, which approximates the conditional probability of unusual zero provided that the corresponding observed interaction is a zero, i.e., equation (22) of the **ZIP-LPCM** paper, by posterior mean of  $\boldsymbol{\nu}$:
...
We follow similar implementations shown above to apply the unsupervised version bringing the **ZIP-LPCM unSup Beta(1,9)** case for **scenario 1** network shown in our **ZIP-LPCM** paper:
...
Thus the **Figure 3** in the **ZIP-LPCM** paper can be recovered by:
...
The post-processing code for the above implementations are also the same as **scenario 1** cases and finally completes the **Table 1** of the **ZIP-LPCM** paper.
...
The 1st row and 2nd row of **Figure 5** in the **ZIP-LPCM** paper can be produced, respectively, following:
...
SS2_Scenario1_Directed_ZIPSBM_P_m0_heatmap <-
  pheatmap(SS2_Scenario1_Directed_ZIPSBM_P_m0_dataframe[order(SS2_Scenario1_Directed_ZIPSBM$z),order(SS2_Scenario1_Directed_ZIPSBM$z)],
           ...
           annotation_colors=list(z_ref=annotation_colors_z_ref),annotation_names_row=FALSE,annotation_names_col=FALSE,annotation_legend=FALSE,
           gaps_row=c(which(diff(sort(SS2_Scenario1_Directed_ZIPSBM$z))!=0)),gaps_col=c(which(diff(sort(SS2_Scenario1_Directed_ZIPSBM$z))!=0)))
SS2_Scenario1_Directed_ZIPSBM_P_m0_heatmap_draw <- cowplot::ggdraw(SS2_Scenario1_Directed_ZIPSBM_P_m0_heatmap[[4]])+ theme(plot.background =element_rect(fill=brewer.pal(9,"Greys")[1]))
...
SS2_Scenario1_Directed_ZIPSBM_Sup_ZIPLPCM_T12k_R1_hat_P_m0_heatmap_draw <- cowplot::ggdraw(SS2_Scenario1_Directed_ZIPSBM_Sup_ZIPLPCM_T12k_R1_hat_P_m0_heatmap[[4]])+ theme(plot.background =element_rect(fill=brewer.pal(9,"Greys")[1]))
...
SS2_Scenario2_Directed_ZIPSBM_P_m0_heatmap <-
  pheatmap(SS2_Scenario2_Directed_ZIPSBM_P_m0_dataframe[order(SS2_Scenario2_Directed_ZIPSBM$z),order(SS2_Scenario2_Directed_ZIPSBM$z)],
           ...
           annotation_colors=list(z_ref=annotation_colors_z_ref),annotation_names_row=FALSE,annotation_names_col=FALSE,annotation_legend=FALSE,
           gaps_row=c(which(diff(sort(SS2_Scenario2_Directed_ZIPSBM$z))!=0)),gaps_col=c(which(diff(sort(SS2_Scenario2_Directed_ZIPSBM$z))!=0)))
SS2_Scenario2_Directed_ZIPSBM_P_m0_heatmap_draw <- cowplot::ggdraw(SS2_Scenario2_Directed_ZIPSBM_P_m0_heatmap[[4]])+ theme(plot.background =element_rect(fill=brewer.pal(9,"Greys")[1]))
...
SS2_Scenario2_Directed_ZIPSBM_Sup_ZIPLPCM_T12k_R1_hat_P_m0_heatmap_draw <- cowplot::ggdraw(SS2_Scenario2_Directed_ZIPSBM_Sup_ZIPLPCM_T12k_R1_hat_P_m0_heatmap[[4]])+ theme(plot.background =element_rect(fill=brewer.pal(9,"Greys")[1]))
...
SS2_Scenario1_Directed_ZIPSBM_Sup_ZIPLPCM_T12k_beta_1_19_R1_hat_P_m0_heatmap_draw <- cowplot::ggdraw(SS2_Scenario1_Directed_ZIPSBM_Sup_ZIPLPCM_T12k_beta_1_19_R1_hat_P_m0_heatmap[[4]])+ theme(plot.background =element_rect(fill=brewer.pal(9,"Greys")[1]))
...
SS2_Scenario2_Directed_ZIPSBM_Sup_ZIPLPCM_T12k_beta_1_19_R1_hat_P_m0_heatmap_draw <- cowplot::ggdraw(SS2_Scenario2_Directed_ZIPSBM_Sup_ZIPLPCM_T12k_beta_1_19_R1_hat_P_m0_heatmap[[4]])+ theme(plot.background =element_rect(fill=brewer.pal(9,"Greys")[1]))
...
Finally, we can also obtain the posterior samples of the individual-level unusual zero probability $p_{z_iz_j}$ and the individual-level Poisson rate $\lambda_{z_iz_j}$ as well as the corresponding posterior mean for the **scenario 1** network:
...
which are better than those of the ZIP-LPCM implementations as expected.

Similarly for the **scenario 2 ZIP-SBM** network:

``` r
# Obtain the individual-level p and lambda for each iteration in scenario 2
SS2_Scenario2_Directed_ZIPSBM_T12k_R1_p <- list()
SS2_Scenario2_Directed_ZIPSBM_T12k_R1_lambda <- list()
for (t in 1:nrow(SS2_Scenario2_Directed_ZIPSBM_T12k_R1$z)){
  Z <- t(t(matrix(SS2_Scenario2_Directed_ZIPSBM_T12k_R1_LSz[t,],75,SS2_Scenario2_Directed_ZIPSBM_T12k_R1$K[t]))==(1:SS2_Scenario2_Directed_ZIPSBM_T12k_R1$K[t]))*1
  SS2_Scenario2_Directed_ZIPSBM_T12k_R1_p[[t]] <- Z%*%SS2_Scenario2_Directed_ZIPSBM_T12k_R1_LSP[[t]]%*%t(Z)
  SS2_Scenario2_Directed_ZIPSBM_T12k_R1_lambda[[t]] <- Z%*%SS2_Scenario2_Directed_ZIPSBM_T12k_R1_LSLambda[[t]]%*%t(Z)
  if ((t%%1000) == 0){cat("t=",t,"\n")}
}
SS2_Scenario2_Directed_ZIPSBM_T12k_R1_hat_p <- Reduce("+",SS2_Scenario2_Directed_ZIPSBM_T12k_R1_p[iteration_after_burn_in])/length(iteration_after_burn_in)
SS2_Scenario2_Directed_ZIPSBM_T12k_R1_hat_lambda <- Reduce("+",SS2_Scenario2_Directed_ZIPSBM_T12k_R1_lambda[iteration_after_burn_in])/length(iteration_after_burn_in)
diag(SS2_Scenario2_Directed_ZIPSBM_T12k_R1_hat_p) <- 0
diag(SS2_Scenario2_Directed_ZIPSBM_T12k_R1_hat_lambda) <- 0
# Compare hat_p and reference p by MAE and sdAE
mean(abs((SS2_Scenario2_Directed_ZIPSBM_T12k_R1_hat_p-SS2_Scenario2_Directed_ZIPSBM_p)[diag(nrow(SS2_Scenario2_Directed_ZIPSBM$Y))==0]))
# 0.03829389
sd(abs((SS2_Scenario2_Directed_ZIPSBM_T12k_R1_hat_p-SS2_Scenario2_Directed_ZIPSBM_p)[diag(nrow(SS2_Scenario2_Directed_ZIPSBM$Y))==0]))
# 0.03252658
# Compare hat_lambda and reference lambda by MAE and sdAE
mean(abs((SS2_Scenario2_Directed_ZIPSBM_T12k_R1_hat_lambda-SS2_Scenario2_Directed_ZIPSBM_lambda)[diag(nrow(SS2_Scenario2_Directed_ZIPSBM$Y))==0]))
# 0.08745794
sd(abs((SS2_Scenario2_Directed_ZIPSBM_T12k_R1_hat_lambda-SS2_Scenario2_Directed_ZIPSBM_lambda)[diag(nrow(SS2_Scenario2_Directed_ZIPSBM$Y))==0]))
# 0.1319726
```

However, considering the corresponding scale of the unusual zero probability $p_{z_iz_j}$ which ranges from 0 to 1, and the Poisson rates $\lambda_{z_iz_j}$ which ranges from 0 to 7, the errors compared to the reference are shown to be not deteriorated a lot for the ZIP-LPCM implementations.
Considering also that the ZIP-SBM is not able to provide network visualization, the gains in the ability of network visualization for the ZIP-LPCM is much more valuable compared to these deterioration.
